...
cd ${PWD}
metadata=$(cat package/metadata.json)
...
PACKAGE_TYPE=$(echo ${metadata} | jq .KPackageStructure | sed 's/\(.*\)/\L\1/;s/\//_/g;s/"//g;s/\(plasma_.*\)_.*/\1/')
...
mkdir -p po
find package/ -name \*.qml -o -name \*.js -o -name \*.cpp | sort \
